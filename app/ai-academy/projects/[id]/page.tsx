...
export default function ProjectWorkspace({params}:{params:{id:string}}){
 ...
 const key='fsb-project-'+project.id;
 ...
 useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(key)||'null');if(saved){setPhase(saved.phase||0);setFiles(saved.files||files);setLog(saved.log||log)}}catch{}},[]);
 useEffect(()=>{localStorage.setItem(key,JSON.stringify({phase,files,log}))},[phase,files,log]);
