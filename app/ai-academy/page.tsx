...
export default function AIAcademy(){
 ...
 return <main className="ai-academy-app">
  ...
  <div className="ai-layout"><aside className="ai-stage-list">{filtered.map(s=><button key={s.id} onClick={()=>setActive(s.id)} className={active===s.id?'selected':''}><span>{s.id}</span><div><b>{s.title}</b><small>{s.level} · {s.topics.length} topics</small><a className="stage-learn" href={"/ai-academy/learn/"+s.id} onClick={e=>e.stopPropagation()}>Learn →</a></div></button>)}</aside>
