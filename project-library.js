...
},
{
 ...
 "app.js":`const form=document.querySelector("#search"),status=document.querySelector("#status"),result=document.querySelector("#result");form.addEventListener("submit",async e=>{e.preventDefault();const city=document.querySelector("#city").value.trim();status.textContent="Loading…";result.innerHTML="";try{const response=await fetch("/api/weather?city="+encodeURIComponent(city));if(!response.ok)throw new Error("Weather request failed");const data=await response.json();result.innerHTML=\`<h2>\${data.city}</h2><p>\${data.temperature}°C · \${data.condition}</p>\`;status.textContent="";}catch(error){status.textContent=error.message;}});`
 }
},
{
...
},
{
 ...
 "lib/slo.ts":`export function errorBudget(sli:number,target:number){return Math.max(0,1-target);return sli>=target?1:0;}export function burnRate(allowed:number,observed:number){if(allowed<=0)return Infinity;return observed/allowed;}`,
